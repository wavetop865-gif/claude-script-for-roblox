...
if not DrawingLib or type(DrawingLib.new) ~= "function" then
    DrawingLib = nil
end
if not DrawingLib then
    return setmetatable({
        New = function()
            error("[GameSneeze Lib] Drawing API is not available on this platform.")
        end,
        Loader = function()
            error("[GameSneeze Lib] Drawing API is not available on this platform.")
        end,
...
do
    ...
    --
    function utility:IsClickInput(input)
        if not input then
            return false
        end
    ...
    function utility:GetMobileScale()
        ...
        local shortest = math.min(camera.ViewportSize.X, camera.ViewportSize.Y)
        if shortest <= 480 then
            return 0.58
        elseif shortest <= 600 then
            return 0.68
        elseif shortest <= 750 then
            return 0.76
    ...
    --
    function utility:ScaleSize(size)
        local scale = utility:GetMobileScale()
        return Vector2.new(
            math.max(280, math.floor(size.X * scale)),
            math.max(320, math.floor(size.Y * scale))
        )
    end
    --
    function utility:ConnectMouseWheel(forwardCallback, backwardCallback)
        if not mouse then
            return
        end
        pcall(function()
            utility:Connection(mouse.WheelForward, forwardCallback)
            utility:Connection(mouse.WheelBackward, backwardCallback)
        end)
    end
    --
    ...
    function utility:LoadImage(instance, imageName, imageLink)
        ...
        local data
        local assetPath = library.folders.assets .. "/" .. imageName .. ".png"
        local hasFileSystem = type(isfile) == "function"
            and type(readfile) == "function"
            and type(writefile) == "function"
        --
        if hasFileSystem then
            pcall(function()
                if type(isfolder) == "function" and type(makefolder) == "function" and not isfolder(library.folders.assets) then
                    makefolder(library.folders.assets)
                end
            end)
            if isfile(assetPath) then
                local ok, fileData = pcall(readfile, assetPath)
                if ok then
                    data = fileData
                end
            elseif imageLink then
                ...
                if ok and remoteData then
                    data = remoteData
                    pcall(writefile, assetPath, data)
                end
            end
        elseif imageLink then
            local ok, remoteData = pcall(function()
...
do
    ...
    function library:Loader(info)
        ...
        local size = utility:ScaleSize(info.size or info.Size or Vector2.new(375,359))
        if utility:GetMobileScale() < 1 then
            theme.textsize = math.max(11, math.floor(theme.textsize + 1))
        end
        ...
        function window:Cursor(info)
            window.cursor = {}
            if utility:IsTouchDevice() and not uis.MouseEnabled then
        ...
        function window:Fade()
            ...
            --
            if window.cursor and window.cursor["cursor"] and window.cursor["cursor"].Transparency ~= nil then
            ...
            if not utility:IsTouchDevice() or uis.MouseEnabled then
        ...
        library.ended[#library.ended + 1] = function(Input)
            if utility:IsClickInput(Input) and window.isVisible and window.dragging then
    ...
    function library:New(info)
		local info = info or {}
        local name = info.name or info.Name or info.title or info.Title or "UI Title"
        local size = utility:ScaleSize(info.size or info.Size or Vector2.new(504,604))
        if utility:GetMobileScale() < 1 then
            theme.textsize = math.max(11, math.floor(theme.textsize + 1))
        end
        local accent = info.accent or info.Accent or info.color or info.Color or theme.accent
    ...
    function library:Page(info)
        ...
        --
        library.began[#library.began + 1] = function(Input)
            if utility:IsClickInput(Input) and window.isVisible and utility:MouseOverDrawing({page_button.Position.X,page_button.Position.Y,page_button.Position.X + page_button.Size.X,page_button.Position.Y + page_button.Size.Y}) and window.currentPage ~= page then
                page:Show()
            end
        end
        --
    ...
    function pages:PlayerList(info)
        ...
        for Index = 1, 1 do
            ...
            --
            library.began[#library.began + 1] = function(Input)
                if utility:IsClickInput(Input) and (button_outline.Visible or button.open) and window.isVisible then
                    if utility:IsClickInput(Input) and window.isVisible and button_outline.Visible then
                    ...
                    elseif utility:IsClickInput(Input) and button.open then
                        button:Close()
                    end
                end
            end
        ...
        --
        library.began[#library.began + 1] = function(Input)
            if utility:IsClickInput(Input) and list_outline.Visible and window.isVisible then
        ...
        --
        library.ended[#library.ended + 1] = function(Input)
            if playerList.scrolling[1] and utility:IsClickInput(Input) then
        ...
        --
        utility:ConnectMouseWheel(function()
    ...
    function sections:Toggle(info)
        ...
        function toggle:Colorpicker(info)
            ...
            --
            library.began[#library.began + 1] = function(Input)
                if utility:IsClickInput(Input) and window.isVisible and colorpicker_outline.Visible then
                ...
                elseif utility:IsClickInput(Input) and colorpicker.open then
                    colorpicker.open = not colorpicker.open
                    --
                    for i,v in pairs(colorpicker.holder.drawings) do
                        utility:Remove(v)
            ...
            --
            library.ended[#library.ended + 1] = function(Input)
                if utility:IsClickInput(Input) then
        ...
        function toggle:Keybind(info)
            ...
            library.began[#library.began + 1] = function(Input)
                ...
                --
                if utility:IsClickInput(Input) and window.isVisible and keybind_outline.Visible then
    ...
    function sections:Slider(info)
        ...
        --
        library.began[#library.began + 1] = function(Input)
            if utility:IsClickInput(Input) and slider_outline.Visible and window.isVisible and page.open and utility:MouseOverDrawing({section.section_frame.Position.X, section.section_frame.Position.Y + slider.axis, section.section_frame.Position.X + section.section_frame.Size.X, section.section_frame.Position.Y + slider.axis + (name and 29 or 14)}) and not window:IsOverContent() then
                slider.holding = true
                slider:Refresh()
            end
        end
        --
        library.ended[#library.ended + 1] = function(Input)
            if utility:IsClickInput(Input) and slider.holding and window.isVisible then
    ...
    function sections:Button(info)
        ...
        --
        library.began[#library.began + 1] = function(Input)
            if utility:IsClickInput(Input) and button_outline.Visible and window.isVisible and utility:MouseOverDrawing({section.section_frame.Position.X, section.section_frame.Position.Y + button.axis, section.section_frame.Position.X + section.section_frame.Size.X, section.section_frame.Position.Y + button.axis + 20}) and not window:IsOverContent() then
                task.spawn(function()
                    utility:LoadImage(button_gradient, "gradientdown", "https://i.imgur.com/DzrzUt3.png") 
                    --
                    task.wait(0.15)
    ...
    function sections:TextBox(info)
        ...
        --
        library.began[#library.began + 1] = function(Input)
            if utility:IsClickInput(Input) and textbox_outline.Visible and window.isVisible then
    ...
    function sections:ButtonHolder(info)
        ...
        for i=1, 2 do
            ...
            --
            library.began[#library.began + 1] = function(Input)
                if utility:IsClickInput(Input) and button_outline.Visible and window.isVisible and utility:MouseOverDrawing({section.section_frame.Position.X + (i == 2 and (section.section_frame.Size.X/2) or 0), section.section_frame.Position.Y + button.axis, section.section_frame.Position.X + section.section_frame.Size.X - (i == 1 and (section.section_frame.Size.X/2) or 0), section.section_frame.Position.Y + button.axis + 20}) and not window:IsOverContent() then
                    task.spawn(function()
                        utility:LoadImage(button_gradient, "gradientdown", "https://i.imgur.com/DzrzUt3.png") 
                        --
                        task.wait(0.15)
    ...
    function sections:Dropdown(info)
        ...
        --
        library.began[#library.began + 1] = function(Input)
            if utility:IsClickInput(Input) and window.isVisible and dropdown_outline.Visible then
            ...
            elseif utility:IsClickInput(Input) and dropdown.open then
                dropdown.open = not dropdown.open
                utility:LoadImage(dropdown_image, "arrow_down", "https://i.imgur.com/tVqy0nL.png")
                --
                for i,v in pairs(dropdown.holder.drawings) do
        ...
        if max then
            library.ended[#library.ended + 1] = function(Input)
                if dropdown.scrolling and dropdown.scrolling[1] and utility:IsClickInput(Input) then
    ...
    function sections:Multibox(info)
        ...
        --
        library.began[#library.began + 1] = function(Input)
            if utility:IsClickInput(Input) and window.isVisible and multibox_outline.Visible then
            ...
            elseif utility:IsClickInput(Input) and multibox.open then
                multibox.open = not multibox.open
                utility:LoadImage(multibox_image, "arrow_down", "https://i.imgur.com/tVqy0nL.png")
                --
                for i,v in pairs(multibox.holder.drawings) do
    ...
    function sections:List(info)
        ...
        --
        library.ended[#library.ended + 1] = function(Input)
            if list.scrolling[1] and utility:IsClickInput(Input) then
